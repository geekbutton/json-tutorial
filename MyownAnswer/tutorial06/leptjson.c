...
static int lept_parse_string_raw(lept_context* c, char** str, size_t* len) {
    ...
    for (;;) {
        ...
        switch (ch) {
            case '\"':
                (*len) = c->top - head;
				memcpy((*str) = (char*)malloc(*len), (c->stack) + (c->top - (*len)),(*len));
				c->top -= (*len);
...
static lept_parse_string(lept_context* c, lept_value* v) {
	int ret;
	char* s;
	size_t len;
...
static int lept_parse_object(lept_context* c, lept_value* v) {
	...
	for (size_t i = 0; i < size; ++i) {
		lept_member* temp_member = ((lept_member*)lept_context_pop(c, sizeof(lept_member)));
		free(temp_member->k);
		lept_free(&temp_member->v);
	}
	return ret;
}
    ...
    switch (v->type) {
		...
		case LEPT_OBJECT:
			for (i = 0; i < v->u.o.size; ++i) {
				lept_member* temp_member = &v->u.o.m[i];
				free(temp_member->k);
				lept_free(&temp_member->v);
			}
			free(v->u.o.m);
